    ...
    class ObjetoAcessoDados {
        ...
        function salvarFornecedor( ModeloFornecedor $fornecedor ) {
            ...
            if ( is_null( $fornecedor->id ) ) {
                $sql = " insert into fornecedor values (default,'$cnpj','$pessoa->id') returning id; ";
                

                $id = $this->conexao->query( $sql )->fetchAll(2)[0]['id'];
            } else {
                $id = $fornecedor->id;
                $sql = " update fornecedor set cnpj = '$cnpj' where id = '$id'; ";
        ...
        function salvarItem( ModeloItem $item ) {
            ...
            if ( is_null( $item->id ) ) {
                $sql = " insert into item values (default,$Fid,$Bid,$Pid,'$data_compra',$quantidade) returning id; ";
                $id = $this->conexao->query( $sql )->fetchAll(2)[0]['id'];
            } else {
                $id = $item->id;
                $sql = "  ";
