...
ini_set('max_execution_time', 60000);  // Aumenta para 60 segundos
    ...
    class ObjetoAcessoDados {
        ...
        function buscarProprietario( $id ) {
            $sql = " select id, cnpj, pessoa from proprietario where id = '$id'; ";
            $resultado = $this->conexao->query($sql)->fetchAll( 2 );
            $banca = new ModeloBanca( $resultado[0]['id'], $resultado[0]['nome'], 
                                     $resultado[0]['pessoa']);
        ...
        function buscarProprietarios() {
            ...
            $sql = " select id, cnpj, pessoa  from proprietario order by cnpj; ";
            $resultado = $this->conexao->query($sql)->fetchAll( 2 );
            $Proprietarios = [];
            // Para cada resultado, busca a banca pelo ID e o adiciona ao array
            foreach ( $resultado as $tupla ) {
        ...
        function salvarProprietario( ModeloProprietario $proprietario ) {
            if ( is_null( $proprietario->id ) ) {
                $id = $proprietario->id;
                $cnpj = $proprietario->cnpj;
                $pessoa = $proprietario->pessoa;

                $sql = " insert into proprietario values (null, '$cnpj', null) returning id; ";
                ...
                $sql = " update proprietario set cnpj = '$cnpj', pessoa = '$pessoa' where id = '$id'; ";
