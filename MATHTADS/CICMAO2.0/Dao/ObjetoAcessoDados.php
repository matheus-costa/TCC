<?php
    ...
    class ObjetoAcessoDados {
        ...
        function buscarCliente( $id ) {
            $sql = " select id, nome, cpf, rg, email, telefone, endereco,  from cliente where id = '$id'; ";

            // Executa a query e obtém o resultado
            $resultado = $this->conexao->query($sql)->fetchAll( 2 );

            // Cria uma nova instância de ModeloAtleta com os dados retornados e o clube associado
            $cliente = new ModeloCliente( $resultado[0]['id'], $resultado[0]['nome'], $resultado[0]['cpf'],$resultado[0]['rg'], $resultado[0]['email'], $resultado[0]['telefone'],$resultado[0]['endereco'], $this->buscarCliente($resultado[0]['venda_produto']));
        ...
        function buscarClientes($id) {
            ...
            $sql = " select id from cliente order by nome; ";
        ...
        // Função para salvar um    $cliente (inserir ou atualizar)
        function salvarCliente( ModeloCliente $cliente ) {
            ...
            $vendaProduto = $cliente->vendaProduto;
            $vendaProdutoId = $venda_produto->id; 
            ...
            if ( is_null( $cliente->id ) ) {
                ...
                $sql = " insert into cliente values (null, '$nome', '$cpf','$rg','$email', '$telefone' ,'$endereco','$vendaProdutoId') returning id; ";
                ...
                $sql = " update cliente set nome = '$nome', cpf = '$cpf',rg = '$rg', email = '$email',telefone = '$telefone', endereco = '$endereco', vendaProduto = '$vendaProdutoId' where id = '$id'; ";
        ...
        // Função para salvar um clube (inserir ou atualizar)
        function salvarCompra( ModeloCompra $compra ) {
            // Se o ID do clube for nulo, insere um novo clube
            if ( is_null( $clube->id ) ) {
                $nome = $banca->nome;
                $numeracao = $banca->numeracao;
        ...
        // Função para remover um clube
        function removerCompra ( ModeloCompra $compra ) {
            // Obtém o ID do clube a ser removido
            $id = $banca->id;
        ...
        function buscarFornecedor( $id ) {
            ...
            $sql = " select id, nome, cpf, rg, cnpj, email, telefone, produto_fornecedor from fornecedor where id = '$id'; ";
            ...
            $fornecedor = new ModeloFornecedor( $resultado[0]['id'], $resultado[0]['nome'], $resultado[0]['cpf'],$resultado[0]['rg'], $resultado[0]['cnpj'], $resultado[0]['email'],$resultado[0]['telefone'], $this->buscarProdutoFornecedor($resultado[0]['produto_fornecedor']) );
        ...
        function buscarFornecedores($id) {
            ...
            $sql = " select id, nome, cpf, rg, cnpj, email, telefone, produto_fornecedor from fornecedor order by nome; ";
        ...
        function salvarFornecedor( ModeloFornecedor $fornecedor ) {
            ...
            $produto_fornecedor = $fornecedor->produto_fornecedor;
            $produtoFornecedorID = $produto_fornecedor->id;
            ...
            if ( is_null( $fornecedor->id ) ) {
                ...
                $sql = " insert into fornecedor values (null, '$nome', '$cpf','$rg','$cnpj',' $email','$telefone','$produtoFornecedorID') returning id; ";

                // Executa a query e obtém o ID gerado
                $id = $this->conexao->query( $sql )->fetchAll(2)[0]['id'];
            } else {
                // Se o fornecedor já existir (ID não for nulo), atualiza o atleta
                $id = $fornecedor->id;

                // Monta a query SQL para atualizar o fornecedor existente
                $sql = " update fornecedor set nome = '$nome', cpf = '$cpf', rg = '$rg', cnpj = '$cnpj', email = '$email', telefone = '$telefone',  produto_fornecedor = '$produtoFornecedorID' where id = '$id'; ";
        ...
        function salvarFuncionario( ModeloFuncionario $funcionario ) {
            ...
            if ( is_null( $clube->id ) ) {
                ...
                $proprietario_chefe = $fornecedor->proprietario_chefe;
                
                // Monta a query SQL para inserir o clube e retornar o ID
                $sql = " insert into funcionario values (null, '$nome','$cpf','$rg','$email', '$telefone','$proprietario_chefe') returning id; ";
                ...
                // Monta a query SQL para atualizar o clube existente
                $sql = " update funcionario set nome = '$nome', cpf = '$cpf', rg = '$rg', email = '$email', telefone = '$telefone', $proprietario_chefe = 'proprietario_chefe'  where id = '$id'; ";
        ...
        function buscarProduto( $id ) {
            // Monta a query SQL para selecionar o atleta pelo ID
            $sql = " select id, nome, tamanho, marca, preco, fornecedor_produto, venda_produto from produto where id = '$id'; ";

            // Executa a query e obtém o resultado
            $resultado = $this->conexao->query($sql)->fetchAll( 2 );

            // Cria uma nova instância de ModeloAtleta com os dados retornados e o clube associado
            $produto = new ModeloProduto( $resultado[0]['id'], $resultado[0]['nome'], $resultado[0]['tamanho'], $resultado[0]['marca'], $resultado[0]['preco'],$resultado[0]['fornecedor_produto'], $resultado[0]['venda_produto'], $this->buscarClube($resultado[0]['clube']) );
